...
export function registerPairingCli(ctx, runtime) {
  const root = ctx.program.command(CHANNEL_ID).description("Manage Mumbletree Chat pairing");
  root.command("pair")
    .requiredOption("--code <code>", "One-time pairing code")
    .option("--server-url <url>", "Mumbletree Chat server URL", DEFAULT_SERVER_URL)
    ...
    .action(async (options) => {
      ...
      await runtime.config.mutateConfigFile({
        afterWrite: { mode: "restart", reason: "Mumbletree Chat pairing updated" },
        mutate(draft) {
          draft.channels ??= {};
      ...
      process.stdout.write(`Paired Mumbletree Chat bot ${result.botId} for group ${result.groupId}.\n`);
    });
}
