...
export function openSocket(url, signal, onEvent, WebSocketImpl = WebSocket) {
  return new Promise((resolve, reject) => {
    ...
    socket.addEventListener("error", () => finish(new Error("Mumbletree Chat WebSocket failed")), { once: true });
  });
}

export async function startGateway(ctx, dependencies = {}) {
  const { account } = ctx;
  if (!account.configured) throw new Error(`Mumbletree Chat account ${account.accountId} is not configured`);
  ...
  while (!ctx.abortSignal.aborted) {
    try {
      ...
      if (!ctx.abortSignal.aborted) throw new Error("Mumbletree Chat WebSocket closed");
    } catch (error) {
