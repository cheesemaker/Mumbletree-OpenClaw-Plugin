...
export const CHANNEL_ID = "mumbletree-chat";
...
export function readCredential(file) {
  ...
  if (!resolved) throw new Error("Mumbletree Chat credential file is not configured");
  const value = JSON.parse(fs.readFileSync(resolved, "utf8"));
  if (typeof value.token !== "string" || !value.token.startsWith("spchat_bot_")) {
    throw new Error("Mumbletree Chat credential file does not contain a valid bot token");
  }
  return value;
}
